...
public class XmlParserServlet extends HttpServlet {
	...
	private void performAction(HttpServletRequest req, HttpServletResponse resp)
		...
		try {
			if (req.getParameterMap().containsKey(AppConstants.ACTION))
				strAction = req.getParameter(AppConstants.ACTION);
			...
			if (strAction.equalsIgnoreCase(AppConstants.GET_RSS)) {
				...
				getChannels(resp);
					...
					.equalsIgnoreCase(AppConstants.GET_MORE_CHANNELS)) {
	...
	private void createChannel(HttpServletRequest req) {
		AppMisc.createNewPost(req.getParameter("name"),
				req.getParameter("rssLink"),
				Integer.parseInt(req.getParameter("ratingCount")));
	}
	...
	private void getMoreChannels(HttpServletResponse resp,
			HttpServletRequest req) throws IOException {
		lstChannels = AppMisc.getMorePosts(req.getParameter("cursorStr")
				.toString());
		...
		for (int index = 0; index < lstChannels.size(); index++) {
			...
			resp.getWriter().print(lstChannels.get(index).getRssLink() + "|");
			resp.getWriter().print(
					lstChannels.get(index).getRatingCount() + "|");
			resp.getWriter().print("~");
		}
		if (!AppMisc.cursorString.isEmpty()) {
			resp.getWriter().print(AppMisc.cursorString);
	...
	private void getMoreRSS(HttpServletResponse resp,
			HttpServletRequest req) throws IOException {
		List<String> lsMain = new ArrayList<String>();
	...
	private List<String> mainParser(String feedId) {
		//feedId = "http://feeds.bbci.co.uk/news/rss.xml";//"http://geo.tv/data/rss/geo_pakistan.xml";// "http://rss.cnn.com/rss/edition.rss";//"http://feeds.bbci.co.uk/news/rss.xml";
		List<String> ls = new ArrayList<String>();
		try {
