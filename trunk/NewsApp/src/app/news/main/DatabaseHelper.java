...
public class DatabaseHelper extends SQLiteOpenHelper {
	...
	public void onCreate(SQLiteDatabase db) {
		db.execSQL("CREATE TABLE " + channelsTable + " (" + colID
				+ " INTEGER PRIMARY KEY AUTOINCREMENT, " + channelName
				+ " TEXT, " + rssLink + " TEXT," + "flag INTEGER)");
	...
	public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
		...
		// db.execSQL("DROP TABLE IF EXISTS "+faltudeptTable);
		//
		// db.execSQL("DROP TRIGGER IF EXISTS dept_id_trigger");
		// db.execSQL("DROP TRIGGER IF EXISTS dept_id_trigger22");
		// db.execSQL("DROP TRIGGER IF EXISTS fk_empdept_deptid");
		// db.execSQL("DROP VIEW IF EXISTS "+faltuviewEmps);
		// onCreate(db);
	}
	...
	public Cursor getChannelsBlock(String id) {
		...
		Cursor cur;
		if( id.equals("0")){
			cur = db.rawQuery("SELECT * FROM " + channelsTable
					+ " LIMIT 20", null); 
		}
		else{
			cur = db.rawQuery("SELECT * FROM " + channelsTable
	...
	/*
	 * Show the channels. so that the user clicks then the headings will be
	 * shown, etc
	 */
	public Cursor getChannelsBlock_Feed(String id) {
		...
		// db.rawQuery("Select "+colID+" as _id , "+colName+", "+colAge+" from "+employeeTable,
		// new String [] {});
		Cursor cur = db.rawQuery("SELECT * FROM " + channelsTable
				+ " WHERE FLAG=1 AND " + colID + " > " + id + " LIMIT 20", null);
		return cur;
	}
	...
	Cursor getAllDepts() {
		...
		Cursor cur = db
				.rawQuery("SELECT " + falutcolDeptID + " as _id, "
						+ faltucolDeptName + " from " + faltudeptTable,
						new String[] {});
...
//		 * FEEDZILLA
//		 */
//		cv.put(channelName, "FeedZilla - Pakistan");
//		cv.put(rssLink,
//				"http://news.feedzilla.com/en_pk/headlines/world-news/asia-pakistan.rss");
//		cv.put(flag, "0");
//		db.insert(channelsTable, channelName, cv);
		...
		cv.put(rssLink,
				"http://feeds.feedburner.com/pakistan-news-newspaper-daily-english-online/Breaking");
		cv.put(flag, "0");
		db.insert(channelsTable, channelName, cv);

		cv.put(channelName, "Nation - Headline");
		cv.put(rssLink,
				"http://feeds.feedburner.com/pakistan-news-newspaper-daily-english-online/headlines");
		cv.put(flag, "0");
		db.insert(channelsTable, channelName, cv);

		cv.put(channelName, "Nation - Today's Headlines");
		cv.put(rssLink,
				"http://feeds.feedburner.com/pakistan-news-newspaper-daily-english-online/today-headlines");
		cv.put(flag, "0");
		db.insert(channelsTable, channelName, cv);

		cv.put(channelName, "Nation - Politics");
		cv.put(rssLink,
				"http://feeds.feedburner.com/pakistan-news-newspaper-daily-english-online/Politics");
		cv.put(flag, "0");
		db.insert(channelsTable, channelName, cv);

		cv.put(channelName, "Nation - Business");
		cv.put(rssLink,
				"http://feeds.feedburner.com/pakistan-news-newspaper-daily-english-online/Business");
		cv.put(flag, "0");
		db.insert(channelsTable, channelName, cv);

		cv.put(channelName, "Nation - International");
		cv.put(rssLink,
				"http://feeds.feedburner.com/pakistan-news-newspaper-daily-english-online/International");
		cv.put(flag, "0");
		db.insert(channelsTable, channelName, cv);

		cv.put(channelName, "Nation - Sports");
		cv.put(rssLink,
				"http://feeds.feedburner.com/pakistan-news-newspaper-daily-english-online/Sports");
		cv.put(flag, "0");
		db.insert(channelsTable, channelName, cv);

		cv.put(channelName, "Nation - Entertainment");
		cv.put(rssLink,
				"http://feeds.feedburner.com/pakistan-news-newspaper-daily-english-online/Entertainment");
		cv.put(flag, "0");
		db.insert(channelsTable, channelName, cv);

		cv.put(channelName, "Nation - Lahore");
		cv.put(rssLink,
				"http://feeds.feedburner.com/pakistan-news-newspaper-daily-english-online/lahore");
		cv.put(flag, "0");
		db.insert(channelsTable, channelName, cv);

		cv.put(channelName, "Nation - Karachi");
		cv.put(rssLink,
				"http://feeds.feedburner.com/pakistan-news-newspaper-daily-english-online/karachi");
		cv.put(flag, "0");
		db.insert(channelsTable, channelName, cv);

		cv.put(channelName, "Nation - Islamabad");
		cv.put(rssLink,
				"http://feeds.feedburner.com/pakistan-news-newspaper-daily-english-online/islamabad");
		cv.put(flag, "0");
		db.insert(channelsTable, channelName, cv);
		...
		cv.put(rssLink,
				"http://feeds.feedburner.com/News-World-Afghanistan-Pakistan");
		cv.put(flag, "0");
		db.insert(channelsTable, channelName, cv);
...
//		db.insert(channelsTable, channelName, cv);
	...
	// Cursor c = db.rawQuery("SELECT " + faltucolDeptName + " FROM"
	// + faltudeptTable + " WHERE " + falutcolDeptID + "=?", params);
	// c.moveToFirst();
	// int index = c.getColumnIndex(faltucolDeptName);
	...
	// SQLiteDatabase db = this.getReadableDatabase();
	// String[] columns = new String[] { "_id", channelName, rssLink,
	// faltucolDeptName };
	// Cursor c = db.query(faltuviewEmps, columns, faltucolDeptName + "=?",
	...
	// SQLiteDatabase db = this.getReadableDatabase();
	// Cursor c = db.query(faltudeptTable, new String[] {
	// falutcolDeptID + " as _id", faltucolDeptName },
	// faltucolDeptName + "=?", new String[] { Dept }, null, null,
	// null);
	// // Cursor
	// //
	...
	public int UpdateChannel(Channel emp) {
		...
		return db.update(channelsTable, cv, colID + "=?",
