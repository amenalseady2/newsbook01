...
else
{
	...
	if ($_GET["userid"] == $_SESSION["userid"])
		...
		$query_get_friends = sprintf("select friendwith from tblfriends where userid = '%s' and friendshipstatus = 2 and friendwith <> '%s'", $_SESSION["userid"], $_SESSION["userid"]);
		$fren_rslt = mysql_query($query_get_friends); 
		
		$main_user_fren = array();
		
		while($frow = mysql_fetch_array($fren_rslt)){
			$main_user_fren[] = $frow["friendwith"];
		}
                        ...
                        <div class="email_table">
							...
							{							
								...
								if(empty($row["profilepic"])){
										$profile_picture = "empty_profile.jpg";	
									}else {
								...
								$search_id=$row['friendwith'];
									...
									//	if((($_SESSION["userid"] != $row["userid"]) && ($row["profilepic"]!="" && $row["access_pic"]!="1")))
											$message=$message."<div class='searchlist'><a class='reqlisttitle' href='viewprofile.php?userid=".$row['friendwith']."'><img src='profilepics/".$profile_picture."' style='background-color:#FFFFFF' width='50'  border='0'></a></div>";
										//else
									...
									$message=$message."<div class='frnd-list-action'>";
										...
										if (in_array($row["friendwith"], $main_user_fren))
											$message=$message."<div class='friend-list-button'>Common Friend</div><div style='width:5px;float:left;'>&nbsp;</div>";
...
/*								
									...
									if($row["profilepic"]!="" && $row["access_pic"]!="1")
										$message=$message."<div class='searchlist'><a class='reqlisttitle' href='viewprofile.php?userid=".$row['friendwith']."'><img src='profilepics/".$row["profilepic"]."' style='background-color:#FFFFFF' width='50'  border='0'></a></div>";
									else
										$message=$message."<div class='searchlist'><a class='reqlisttitle' href='viewprofile.php?userid=".$row['friendwith']."'><img src='profilepics/empty_profile.jpg' style='background-color:#FFFFFF' width='50' height='50' border='0'></a></div>";
...
//												</div>";
									...
									{
										$message=$message."<div class='friend-list-button' ".$visible_style.">";
